...
class DCGCN():
    def __init__(self, hparams, mode):
        ...
        with tf.variable_scope("convolution-block") as scope:
            ...
            output = tf.layers.dense(tf.concat(gcn_layers, axis=-1), 1,activation=tf.nn.softmax, use_bias=True)
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            if self.gcn_encoding:
                    self.enc_seq_embs =self.enc_seq_embs+ self.enc_char_emb_gcn
            if self.is_training:
    ...
    def conv_layer(self, input, sequence_length, filter_size, embedding_size, num_filters):
        with tf.name_scope("conv_layer"):
            ...
            op = tf.nn.max_pool(act, ksize=[1, sequence_length - filter_size + 1, 1, 1], strides=[1, 1, 1, 1],
                                padding='VALID')
