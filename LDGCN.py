...
class LDGCN():
    def __init__(self, hparams, mode):
        ...
        self.gcn_encoding=hparams.gcn_encoding
        self.gcn_num_hidden=hparams.gcn_num_hidden
        self.gcn_num_layers=hparams.gcn_num_layers
        self.gcn_dropout=hparams.gcn_dropout
        ...
        with tf.variable_scope("project"):
            ...
            self.input_conv = layers_core.Dense(self.num_units/self.gcn_num_layers, use_bias=False)
        ...
        with tf.variable_scope("convolution-block") as scope:
            ...
            portion1 = tf.expand_dims(portion1, -1)
            portion2 = tf.expand_dims(portion2, -1)
            portion3 = tf.expand_dims(portion3, -1)
            portion4 = tf.expand_dims(portion4, -1)
            gcn_layers = []
            input = []
            output_layerwise1 = []
            output_layerwise2 = []
            input_deep = []
            input.append(portion1)
            input.append(portion2)
            input.append(portion3)
            ...
            for i in range(0, 2):
                ...
                output_layerwise1.append(output)
            for i in range(2, 4):
                conv = tf.nn.conv2d(input[i],
                                    W1,
                                    strides=[1, 1, 1, 1],
                ...
                output = tf.nn.relu(tf.nn.bias_add(conv, b1), name="relu")
                output = tf.layers.dropout(output, rate=self.gcn_dropout)
                output_layerwise2.append(output)
            group1 = tf.concat(output_layerwise1, axis=1)
            group2 = tf.concat(output_layerwise2, axis=1)
            input_deep.append(group1)
            input_deep.append(group2)
            for i in range(0, 2):
                conv = tf.nn.depthwise_conv2d(input_deep[i],
                                              W1,
                                              strides=[1, 1, 1, 1],
                                              padding="SAME",
                                              name="conv1")
