...
def infer(hparams):
    ...
    to_vocab, rev_to_vocab = ReadingData.initialize_vocabulary(hparams.from_vocab)
    outputs = []
    ...
    if epoch and tf.train.checkpoint_exists(epoch.model_checkpoint_path):
        ...
        f1 = open("Reference2(LDGCN)", "w", encoding="utf-8")
        f2 = open("Prediction2(LDGCN)", "w", encoding="utf-8")
        ct = 0
